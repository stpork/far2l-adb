...
// ASCII-only fold for filename/extension compares; Unicode case folding
// would change byte counts and is unnecessary for the formats we touch.
...
void EnsureLibmtpInit() {
    ...
    std::call_once(once, []() {
        // Redirect stderr to plugin dir so both mtp.log and mtp_libmtp.log
        // live next to the plugin binary, making them easy to find.
        ...
        // Default USB+PTP traces: enough for claim/OpenSession/IO debug.
        // MTP_LIBMTP_DEBUG=all|data|usb|ptp to dial up/down.
        int mask = LIBMTP_DEBUG_USB | LIBMTP_DEBUG_PTP;
...
ObjectEntry FromMtpFile(const LIBMTP_file_t* f) {
    ...
    e.format = static_cast<uint16_t>(f->filetype);
    return e;
}
...
// USB SET_CONFIGURATION resets kernel-side claims so icdd/gvfs-mtp's
// exclusive-access drops; ~300ms settle, then libmtp opens. MTP_NO_KICK=1.
#if defined(__APPLE__)

// macOS: iSerialNumber via device-level libusb_open. icdd holds the
// *interface*; the device itself is still openable (control via ep 0).
...
// Image Capture per-device pref key: "Device-VVVV-PPPP-SERIAL"
// (uppercase hex), or "Device-VVVV-PPPP" when serial is unavailable.
...
// Equivalent to Image Capture.app's "Connecting this device opens: No
// application" — icdd reads at match-time and skips the claim.
...
// Per-user MTP-claimer pids (icdd, mscamerad-xpc, ptpcamerad) via
// sysctl. adb's fork-server is left alone (USB-debugging conflict).
struct ClaimerPids {
    pid_t icdd = 0;
    pid_t mscamerad = 0;
    pid_t ptpcamerad = 0;
...
// Write "No app" pref → SIGKILL icdd/* → 50ms settle (respawned icdd
// reads pref and skips the claim). MTP_NO_ICDD_BYPASS=1 disables.
void DislodgeIcdd(libusb_context* ctx, int bus, int address,
    ...
    if (killed) {
        // IOKit-cleanup window before respawned icdd re-claims.
        usleep(50 * 1000);
    }
}
...
void KickPriorInterfaceClaim(int bus, int address) {
    ...
    for (ssize_t i = 0; i < cnt; ++i) {
        ...
        if (orc != 0 || !h) {
            // Most likely icdd holds the device with kIOUSBExclusiveAccess
            // at IOKit level; can't even open it, let alone kick.
            break;
        }
        int cfg = -1;
    ...
    if (kicked) {
        // Settle: device altsetting/endpoint reshuffle after
        // SetConfiguration before libmtp opens + PTP-OpenSessions.
        usleep(300 * 1000);
    }
}
...
Result<std::vector<UsbDeviceCandidate>> LibMtpBackend::Enumerate() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        // Concrete USB descriptors beat libmtp's catchall ("Galaxy
        // models (MTP)"); device-level open works through icdd's grip.
...
void LibMtpBackend::ReleaseShared(const std::string& device_key) {
    ...
    // Keep weak_ptr alive while another panel holds the backend; second
    // libmtp session on same device fails "device busy".
...
Status LibMtpBackend::DrainErrors(const char* what) {
    ...
    // libmtp drops PTP RCs; pull from message and treat "device refused"
    // RCs as Unsupported so host fallback covers them.
...
Status LibMtpBackend::Connect() {
    ...
    // Single open attempt; libmtp leaks claim state on later-step
    // failure. Surface real cause via MTP_LIBMTP_DEBUG=usb+ptp.
#if defined(__APPLE__)
    ...
    // Uncached: cached mode's initial flush_handles takes 5+ minutes on
    // a Galaxy with tens of thousands of photos.
    ...
    // 6×250ms retry: some devices haven't published the storage tree
    // immediately after Open. Non-fatal on giveup.
    ClearErrors();
...
// No PollEvents: libmtp has no cheap presence ping; disconnect surfaces
// lazily at next user op (NO_DEVICE_ATTACHED → Disconnected).
...
void LibMtpBackend::ReopenAfterCancelLocked() {
    // Release only; eager reopen blocks _mtx and didn't recover Sony's
    // post-CancelRequest desync. Next user op triggers Connect().
    if (_device) {
...
Result<std::vector<StorageEntry>> LibMtpBackend::ListStorages() {
    ...
    for (LIBMTP_devicestorage_t* s = _device->storage; s; s = s->next) {
        ...
        // 0x0003 FixedRAM = Internal, 0x0004 RemovableRAM = External;
        // ROM variants (0x0001/0x0002) → Unknown.
...
Result<std::vector<ObjectEntry>> LibMtpBackend::ListChildrenLocked(StorageId storageId, ObjectHandle parent, bool include_tombstones) {
    ...
    while (head) {
        ...
        // Samsung quirk: blank filename for in-session-created objects
        // (catalog doesn't republish until reconnect); patch from cache.
        if (e.name.empty()) {
            auto it = _known_names.find(e.handle);
            if (it != _known_names.end()) {
                e.name = it->second;
            }
        }
        out.push_back(std::move(e));
...
Result<ObjectEntry> LibMtpBackend::StatLocked(ObjectHandle handle) {
    ...
    // Galaxy quirk patch (see ListChildrenLocked).
    if (e.name.empty()) {
        auto it = _known_names.find(e.handle);
        if (it != _known_names.end()) {
            e.name = it->second;
...
Status LibMtpBackend::Upload(const std::string& local_path,
    ...
    // PickUploadFiletype: Sony rejects ObjectFormat=Undefined, so always
    // pass a type the device advertises in SupportedFiletypes.
    ...
    LIBMTP_destroy_file_t(meta);
    if (rc == 0 && new_handle) {
        // Cache the upload's filename — Galaxy returns blank from
        // Get_Files_And_Folders for newly-created handles in this session.
...
Status LibMtpBackend::Rename(ObjectHandle handle, const std::string& new_name) {
    ...
    // Update cached name so subsequent listings show the new value
    // (if the old one was already cached, replace; if not, add).
...
Status LibMtpBackend::DeleteRecursiveLocked(ObjectHandle handle,
    ...
    if (stat.value.is_dir) {
        // MTP spec: Delete_Object on a folder recursively deletes all
        // descendants in one command. Try the fast path first.
        ClearErrors();
        ...
        ClearErrors();
        // Fallback: device doesn't recursive-delete OR Galaxy tombstones
        // (ghost children from prior deletes block the folder).
        ...
        // Re-list to flush libmtp / device-side cache before the
        // parent Delete_Object so it sees the folder as actually empty.
...
Result<ObjectHandle> LibMtpBackend::MakeDirectory(const std::string& name, StorageId storageId, ObjectHandle parent) {
    ...
    // Samsung quirk: SendObjectInfo with parent=0 → InvalidObjectHandle;
    // wants 0xFFFFFFFF for "create at storage root". Normalize here.
...
Result<ObjectHandle> LibMtpBackend::CopyObject(ObjectHandle handle, StorageId storageId, ObjectHandle parent) {
    ...
    // libmtp's wrapper drops the new handle (PTP returns it in param[0]).
    // Recover by listing target and matching name ≠ source handle.
    ...
    // Copy succeeded but we couldn't relocate the new handle; caller's
    // subsequent Rename (Shift+F5) will fail and surface it.
...
Status LibMtpBackend::MoveObject(ObjectHandle handle, StorageId storageId, ObjectHandle parent) {
    ...
    const uint32_t pid = (parent == kRootParent || parent == 0) ? 0xFFFFFFFFu : parent;
    ClearErrors();
    if (LIBMTP_Move_Object(_device, handle, storageId, pid) != 0) {
        return DrainErrors("Move_Object");
    }
    return OkStatus();
}
...
void LibMtpBackend::EnsureCapsProbedLocked() {
    ...
    _caps_probed = true;
    if (!_device) return;
    // Capability-driven, no vendor / product table — we ask libmtp,
    // which queries the device's published OperationsSupported list.
...
bool LibMtpBackend::StorageIsCaseInsensitive(StorageId storageId) {
    ...
    for (LIBMTP_devicestorage_t* s = _device->storage; s; s = s->next) {
        ...
        // StorageType is the reliable discriminator (FilesystemType
        // varies); 0x0004 RemovableRAM = SD card = FAT-class.
        return s->StorageType == 0x0004;
    }
    return false;
}
...
uint16_t LibMtpBackend::PickUploadFiletype(const std::string& remote_name) {
    ...
    static const Map kMap[] = {
        ...
        // libmtp 1.1.x has no PDF enum; map to DOC which all tested
        // responders accept as a generic document.
    ...
    // Either we couldn't map the extension OR the device rejects this
    // format. Try Undefined — most permissive (Galaxy etc. accept it).
    ...
    // Sony rejects Undefined too: pick a "neutral" advertised type so
    // bytes still land — filename's extension drives on-device apps.
