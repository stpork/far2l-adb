...
struct ProgressState {
    ...
    // Fires once on aborting false→true; callers forward to whatever
    // blocks the worker (e.g. libmtp progress trampoline cancel).
...
// Mirrors far2l's WarnCopyDlg layout (copy.cpp:3017) — minus
// Append/Resume/Rename which don't apply to MTP transfers.
...
// Mirrors far2l's CopyProgress::CreateBackground; single-file = 9
// content rows, multi-file = 11 (adds Total bar + separator).
...
private:
    ...
    // Esc gate: ignore Esc within ~200ms of show (drops buffered keys +
    // F-key-emitted-Esc that would otherwise abort instantly).
...
public:
    ...
    // Title + body, body word-wrapped. Avoids Far's right-edge truncation
    // on long libmtp error strings.
...
// Mirrors far2l's shell-delete UI: small centered modal, current item
// name debounced (repaints only on worker advance to next item).
