...
public:
    ...
    // Refcounted session: Acquire reuses existing per device-key, Release
    // drops a panel's claim (handle dies with last ref).
    ...
    // Capability accessors — driven by the device's OperationsSupported
    // / SupportedFiletypes, never by vendor/product tables.
    ...
    // Pick a filetype the device accepts for `remote_name`. Falls back to
    // a "neutral" type (Text/HTML/etc.) on devices that reject UNKNOWN.
    uint16_t PickUploadFiletype(const std::string& remote_name);

    // True for FAT-class storages; collision checks must use this so
    // "Test.txt"/"test.txt" collide on SD cards.
...
private:
    ...
    void ClearErrors();
    // After a cancelled USB transfer libmtp's PTP session is desynced
    // (half-committed abort); tear down + reopen for next call.
    ...
    // Samsung-quirk patch: some firmwares return blank filename for
    // session-created handles; we track names client-side.
