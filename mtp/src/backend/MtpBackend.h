...
class MtpBackend final : public IProtocolSession {
public:
    explicit MtpBackend(UsbDeviceCandidate candidate);
    ~MtpBackend() override;

    BackendKind Kind() const override { return BackendKind::MTP; }
    ...
    Result<ObjectHandle> CopyObject(ObjectHandle handle, StorageId storageId, ObjectHandle parent) override;
    Status MoveObject(ObjectHandle handle, StorageId storageId, ObjectHandle parent) override;
...
private:
    ...
    std::unique_ptr<MtpSession> _mtp;
