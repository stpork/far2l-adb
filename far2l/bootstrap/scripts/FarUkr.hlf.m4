...
Якщо не вказан ключ -co, то завантажаться плагіни з основного каталогу та з каталогу,
вказаного у параметрі "~Шлях до персональних плагінів~@PluginsManagerSettings@"
 ...
 #-u <username>#
 Дозволяє використовувати різні настройки для різних користувачів (it override #FARSETTINGS# ~environment variable~@FAREnv@ value).
...
змінюється не так на активної, але в пасивної панелі.
 ...
 (без урахування стану командного рядка)
 Помістити повні імена позначених файлів #Ctrl-Alt-Ins#
...
через їх десяткові коди.
 ...
 3. ^<wrap>Комбінація клавіш #Ctrl-Alt-Ins# працює за такими правилами:
    ^<wrap>Якщо в ~Налаштуваннях панелі~@PanelSettings@ увімкнено опцію
"Класичне розширення посилань гарячими клавішами", то використовується повне ім’я
з розгортанням ~символічних~@HardSymLink@ посилань.
...
У режимі ~швидкого пошуку~@FastFind@ #Ctrl-Enter# не вставляє ім'я
 ...
 Вставити повне ім'я файлу з активної панелі #Ctrl-F#
 ...
 (*5)
 Вставити повне ім'я файлу з активної панелі #Ctrl-Alt-F#
 Вставити повне ім'я файлу з пасивної панелі #Ctrl-Alt-;#
 Вставити шлях із лівої панелі #Ctrl-Alt-[#
 Вставити шлях із правої панелі #Ctrl-Alt-]#
 Вставити шлях із активної панелі #Alt-Shift-[#
 Вставити шлях із пасивної панелі #Alt-Shift-]#
...
виділяють блок у командному рядку незалежно від стану панелей.
 ...
 5. ^<wrap>Позначені комбінації клавіш працюють за наступними правилами:
    ^<wrap>Якщо в ~Налаштуваннях панелі~@PanelSettings@ увімкнено опцію
"Класичне розширення посилань гарячими клавішами", то використовується повне ім’я
з розгортанням ~символічних~@HardSymLink@ посилань.
...
допустимого розміру консольного вікна та назад.
...
Кошик (Trash). Комбінація клавіш #Shift-Del# завжди видаляє файли,
минаючи Кошик.

 2. ^<wrap>У процесі знищення файл перезаписується нулями (можна задати
інший символ-заповнювач, System.WipeSymbol в ~far:config~@FarConfig@), усікається до нульової довжини,
...
$ #Команда управління панелями - інші команди#
 Копіювання тексту з екрана #Alt-Ins#
   Ця команда дозволяє вибрати та помістити в Буфер Обміну будь-яку область екрану.
   Для переміщення курсору використовуйте клавіші керування курсором або
натисніть ліву кнопку миші.
   Для вибору тексту використовуйте клавіші керування
#курсором# при натиснутій #Shift# або натиснутою #ліву кнопку миші#.
   Комбінація #Ctrl-A# або #A# вибирає весь екран.
   Комбінація #Ctrl-U# або #U# знімає виділення.
   #Enter#, #Ctrl-Ins#, #права кнопка миші# або подвійне натискання #лівої кнопки миші#
копіюють вибраний текст у Буфер Обміну і завершує операцію.
   #Ctrl-<Gray +># додає вибраний текст до поточному змісту Буфера Обміну
і завершує операцію.
   #Esc# скасовує позначку і завершує операцію.
...
(only in GUI backend mode under X11).
...
над папками.
 ...
 FAR2L запам'ятовує інформацію про структуру папок у своїй папці у тимчасовій директорії
системи (/tmp, /var/tmp або заданій змінній $TMPDIR). За потреби можна актуалізувати
стан дерева поєднанням клавіш #Ctrl-R#.
...
$ #Панель інформації#
 ...
 1. ^<wrap>#Мережеві# імена комп'ютера та користувача.

 2. ^<wrap>Відомості про поточний каталог та його файлову систему.
    ^<wrap>Тип файлової системи, загальний обсяг та обсяг простору, доступного непривілейованому користувачу, ідентифікатор
файлової системи, поточний каталог та повний шлях до нього з урахуванням символьних посилань, точка монтування файлової
системи поточного каталогу, максимальна допустима довжина імені файлу для даного типу ФС та прапори, з якими файлова
система змонтована.

 3. ^<wrap>Інформація про пам'ять.
    ^<wrap>Рівень завантаження #пам'яті# (100% означає, що використана вся пам'ять), загальний обсяг доступної оперативної
пам'яті, обсяг вільної пам'яті, обсяг спільного використання пам'яті, обсяг пам'яті, використаної буферами, загальний обсяг
області підкачки, обсяг вільного простору в області підкачки.

 4. ^<wrap>Інформація EditorConfig (якщо доступно).
    ^<wrap>Розташування кореневого та найближчого файлів #.editorconfig#, а також значення властивостей #indent_style#,
#indent_size#, #end_of_line#, #charset#, #trim_trailing_whitespace#, #insert_final_newline# для маски #[*]#.
    ^<wrap>Детальніше про EditorConfig дивіться на офіційному сайті: ~https://editorconfig.org/~@https://editorconfig.org/@.

 5. ^<wrap>Короткий статус Git (якщо доступно).
    ^<wrap>При знаходженні в локальному репозиторії/робочому дереві Git буде показано шлях до кореневої директорії Git та
результат виконання команди #git status -s -b#.

 6. ^<wrap>Файл #опису папки#.

    ^<wrap>Ви можете переглянути вміст цього файлу у повноекранному режимі, натиснувши
...
перегляду файлу опису.
    ...
    ^<wrap>Список можливих імен файлів опису папок може бути заданий за допомогою
команди "Файли опису папок" у ~Меню параметрів~@OptMenu@.

 7. ^<wrap>Панель плагіна
    ^<wrap>Містить інформацію про протилежну панель плагіна, якщо вона надається плагіном.

 Також: ^<wrap>~Налаштування інформаційної панелі~@InfoPanelSettings@
        ^<wrap>~Список макроклавіш~@KeyMacroInfoList@, доступних на панелі інформації.
...
#Швидкий перегляд# Змінити панель на ~панель швидкого перегляду~@QViewPanel@.
 ...
 #Меню переходу# Показати діалог ~Меню переходу~@DriveDlg@ для зміни поточного розташування у файловій системі або відкриття панелі плагіна.
...
#і групи сортування# ~розмальовки файлів та груп сортування~@Highlight@.
 ...
 #Налаштування повідомлень# Викликає діалог ~Налаштування повідомлень~@NotificationsSettings@.
...
підтримка клавіатурних комбінацій (may be #GUI|WX# based on wxWidgets or #GUI|SDL#).
...
Персональні плагіни не завантажуватимуться, якщо в параметрах ~командного рядка~@CmdLine@ вказан ключ -co.
...
вставити нове вкладене меню або пункт меню за допомогою #Ins# чи #Ctrl+N# та редагувати
...
$ #Системні параметри#
  ...
  Якщо ця опція увімкнена, FAR2L буде запитувати пароль sudo при спробі доступу до файлів, що вимагають прав root.

  #Підтверджувати усі операції запису#
  Якщо ця опція увімкнена, FAR2L буде запитувати підтвердження для операцій модифікації під час роботи з підвищеними привілеями.

 #Видаляти в Кошик#
Дозволяє видалити файли за допомогою Кошика (Trash). Операція видалення в
...
каталогів та визначення сумарного розміру файлів у підкаталогах.
  ...
  Цей параметр визначає, як FAR2L обчислює загальний розмір каталогу під час побудови дерева каталогів.
...
two different directories, both entries will be saved in the history.
 ...
 Дозволити FAR2L автоматично призначати однокнопкові гарячі клавіші для елементів у
~Історії команд~@History@, ~Історії папок~@HistoryFolders@ та ~Історії перегляду~@HistoryViews@.
Це може бути зручно, але також існує ризик випадкового вибору через непередбачені натискання
в зв'язку з динамічною природою подібних списків.
 Якщо ви не використовуєте цю функцію або почуваєтеся незручно з нею, ви можете її вимкнути.

 #Автозапис конфігурації#
 Якщо цю опцію увімкнено, FAR2L буде автоматично зберігати конфігурацію при виході з FAR.
Також зберігатимуться поточні папки обох панелей.

 #Автозбереження стану панелей#
 Стан панелей автоматично зберігається при виході з FAR.
...
#папок розширення# не тільки до файлів, але і до папок. При
 ...
 Значення параметру, рівне нулю відповідає стану "автоматичне
 оновлення  увімкнено завжди". Для примусового оновлення панелі
...
додатками. Врахуйте, що ця опція працює тільки в режимі GUI under X11
...
роздільники для дати та часу відповідно до ваших уподобань.
 Кнопка "Reset to default" повертає налаштування до стандартних значень, які
пропонуються far2l.
 Кнопка "Reset to current" буде корисною, якщо ви хочете скасувати зміни, які ще
не були підтверджені, і повернутися до поточних налаштувань far2l.
 Кнопка "From system locale" встановлює формат дати та часу згідно з налаштуваннями
локалі вашої операційної системи.

 #Інтервал миготіння курсору# (*тільки GUI-бекенд)
 Дозволяє зменшити або збільшити частоту миготіння курсора; допустимий діапазон
значень - від 100 до 500 мс.

 #Вибрати шрифт# (*тільки GUI-бекенд)
 Показує діалог вибору шрифту, який використовується для відображення інтерфейсу far2l.

 #Відключити згладжування# (*тільки GUI-бекенд)
 Вимкнення алгоритмів згладжування може трохи прискорити рендеринг, але водночас
погіршити візуальне сприйняття тексту.

 #Use OSC52 to set clipboard data# (*тільки TTY-бекенд)
 OSC52 дозволяє копіювати з far2l, що працює в режимі TTY (навіть через підключення по SSH),
до буфера обміну вашої локальної системи.
 Деяким терміналам також потрібно ввімкнення OSC52 у налаштуваннях терміналу.
 Якщо ви використовуєте far2l на віддаленій небезпечній системі, надання доступу до запису в
буфер обміну може бути потенційно небезпечним.
 Примітка: опція відображається, якщо інші переважні способи доступу до буфера обміну
(TTY|X, TTY|F) недоступні.

 #Override base colors palette# (*тільки TTY-бекенд)
...
клавіш #Ctrl-End#. Операція автозавершення
 ...
 #Показувати символи » «#     Коли текст у полі вводу занадто довгий,
 #при переповненні тексту#    щоб уміститися повністю, на краях поля
                            з’являються маркери » «, що вказують на
                            прихований вміст. Цей параметр дозволяє
                            увімкнути чи вимкнути показ цих маркерів.
...
$ #Налаштування меню#
 ...
 Увімкніть цю опцію для циклічної прокрутки вертикальних меню при утриманні клавіш
стрілок. Досягнувши верхнього або нижнього елемента, курсор автоматично перескочить
на протилежний кінець списку.
...
виконання макросу.
 ...
 Увімкнення відображення розділових ліній між виводами команд у вбудованому ~терміналі~@Terminal@.
 При успішному виконанні команди виводиться зелена лінія з символів "-", а при помилці —
жовта лінія з символів "~~". Це робить вивід більш структурованим і допомагає швидше
оцінювати результати виконання команд.

 #Очікувать натискання перед закриттям#
 Чекати на натиск будь-якої клавіші після виконання команди у вбудованому ~терміналі~@Terminal@
перед відображенням панелей. Допустимі значення: Ніколи/При помилці/Завжди.
...
Ця опція дозволяє змінити ~формат запрошення~@CommandPrompt@ командного рядка.
 ...
 Примусово використовувати вказану командну оболонку в вбудованому ~терміналі~@Terminal@.
 Якщо оболонка не задана, far2l спробує використати системну (#$SHELL#). Якщо системна оболонка
не відповідає внутрішнім критеріям far2l, буде використовуватися #bash#.
 Дізнатися поточну командну оболонку far2l можна за допомогою ~псевдокоманди~@SpecCmd@ #far:about#.
...
спеціальні коди:
 ...
 $p - поточний шлях
 $r - поточний шлях, без скорочень
 $u - ім'я користувача
 $n - ім'я комп'ютера
 $q - символ =
 $s - пробіл
 $t - поточний час у форматі HH:MM:SS
 $z - ім'я Git бранча оточене '{' и '} '; або пуста строка, якщо не вдалося отримати ім'я бранча
 $$ - символ $
...
містити ім'я комп'ютера, поточний шлях, ## / $
...
також список ~макроклавіш~@KeyMacroEditList@, доступних у редакторі.
 ...
 2. ^<wrap>Введення тексту або натискання клавіш #BS#/#Del# застосовується
до кожного рядка вибраного вертикального блоку.

 3. ^<wrap>Alt-код символу на додатковій клавіатурі вставляє в
редагований рядок символ із зазначеним кодом (0-65535).

 4. ^<wrap>Якщо блок не виділено, то #Ctrl-Ins#/#Ctrl-C# позначають поточну
...
Для нового файлу кодова сторінка вибирається залежно від
~налаштувань редактора~@EditorSettings@. За необхідності з #списку#
...
перевіряється, unless #[x] Продовжувати обробку# is set in the group
...
$ #Налаштування повідомлень#
 ...
 Відправляти сповіщення на робочий стіл по завершенні тривалих операцій, таких як
копіювання, переміщення та пошук файлів.

 #Повідомляти про завершення консольної команди#
 Відправляти сповіщення на робочий стіл про успішне виконання команди у вбудованому
~терміналі~@Terminal@ або в разі її невдачі.

 #Повідомляти лише коли у фоні#
 Відстежувати стан вікна far2l і надсилати сповіщення на робочий стіл тільки тоді,
коли воно не активне. Працює як в графічній, так і в термінальній версії far2l.
...
@FileAttrDlg
$ #Файлові атрибути#
    Ця команда може застосовуватися як до окремих файлів, так і до груп файлів
і папок, і дозволяє переглядати та змінювати права доступу, власників,
часові мітки та деякі файлові атрибути.
    Якщо вам не потрібно обробляти файли у вкладених
папки, скасуйте параметр "Обробляти вкладені папки".

    Усього в діалозі 5 секцій.

    1. ^<wrap>#Опис#
       ^<wrap>Тип поточного об'єкта, як його визначає команда #file#.
       ^<wrap>Якщо поточним об'єктом є символічне посилання, то з'являється можливість
перемикатися між "описом", значенням символічного посилання ("#Посилання#") та його розв'язаним
абсолютним шляхом ("#Об'єкт#"). Поле "Посилання" доступне для редагування.

    2. ^<wrap>#Володіння#
       ^<wrap>Дозволяє змінювати користувача та/або групу, які володіють вибраними файлами. Виберіть
потрібні імена з відповідних випадаючих списків.
...
стану:
       ...
       ^<wrap> #[x]# - атрибут встановлений для всіх об'єктів
       ^<wrap>       (Встановити у всіх)
       ^<wrap> #[ ]# - атрибут скинутий для всіх об'єктів
       ^<wrap>       (Скинути у всіх)
       ^<wrap> #[?]# - атрибут не у всіх об'єктів
       ^<wrap>       (Не змінювати цей атрибут)

       ^<wrap>Якщо виділені об'єкти мають однакові атрибути, то контрольні
...
всі контрольні перемикачі будуть 3-х позиційними.
       ^<wrap>Якщо стан контрольних перемикачів на момент виконання команди
установки атрибутів не змінилося (порівняно з початковим станом), то
зазначений атрибут не змінюватиметься.

       ^<wrap>Якщо серед виділених об'єктів немає папок, то опція "Обробляти вкладені
папки" буде недоступною.

    4. ^<wrap>#Атрибути / Прапори#
       ^<wrap>Дозволяє встановити або скинути атрибути "Незмінний", "Тільки для дозаписування",
а також "Прихований" (*тільки macOS та BSD) у вибраному файлі.

    5. ^<wrap>#Дата і час#
       ^<wrap>Підтримуються три різних часу файлу:

       ^<wrap> - час останнього доступу (atime);
       ^<wrap> - час останньої модифікації (mtime);
       ^<wrap> - час зміни статусу (ctime);

       ^<wrap>Якщо ви не бажаєте змінювати час файлу, залиште відповідне поле
порожній. Ви можете натиснути кнопку #Скидання#, щоб очистити всі поля з
...
незмінними. Кнопка #Поточне# дозволяє заповнити поля часу файлу поточним
часом. Кнопка #Вихідне# заповнює поля часу файлу оригінальними
значеннями (доступний для одиночного файлового об'єкта).

       ^<wrap>Зверніть увагу, що 'час зміни статусу' призначений лише для перегляду і
не може бути змінений.
       ^<wrap>Для дисків з файловою системою FAT години, хвилини та секунди часу
останнього доступу завжди дорівнюють нулю.

    #Зверніть увагу, що деякі операції можуть вимагати прав суперкористувача.#
    Ви повинні переконатися, що підвищення привілеїв дозволено в діалозі
~Системних параметрів~@SystemSettings@, або far2l має бути запущений як root.
...
(#ЛівийCtrl-2#) і так далі.
    ...
    Після вибору режиму з'являється діалог, у якому
...
в основному для ~модулів, що підключаються~@Plugins@);
 ...
 S[C,T,F,E,A] - розмір файлу
 P[C,T,F,E,A] - упакований розмір файлу
 G[C,T,F,E,A] - розмір потоків файлу
 де: C – форматувати розмір файлу;
     T - використовувати 1000 замість 1024 як дільник;
     F - відображати розмір файлу в стилі Windows
         Explorer (тобто 999 байт будуть показані
...
або його початкова копія буде перезаписана.
 ...
 Випадний список "Символічні посилання" пропонує три способи копіювання зустрічених символьних посилань:
 - #Завжди копіювати посилання#
 Усі символьні посилання копіюються "як є", без жодної адаптації до нового розташування.
 - #Розумно копіювати посилання або файл#
 Якщо посилання вказує на файл, що підлягає копіюванню в рамках тієї ж операції, то під
час копіювання воно буде змінене так, щоб посилатися на новий екземпляр цього файлу.
Інакше під ім'ям посилання буде скопійовано оригінальний файл.
 - #Копіювати як файл#
 Усі символьні посилання будуть перетворені на звичайні файли, що містять точну
копію файлу, на який вони вказують.
...
та #символічні посилання# (SymLink) для файлів та папок за допомогою команди Alt-F6#.
...
(NT 6.0). Символічні посилання можуть вказувати як на папки, так і на файли як на
локальні, так і на мережеві, у своїй підтримуються відносні шляхи.
...
Наприклад, 'cat !.!' буде виводити на екран всі помічені файли
...
системи:
 ...
 Очищає екран вбудованого ~терміналу~@Terminal@.
...
поточний каталог path.
 ...
 Закриває фонову оболонку вбудованого ~терміналу~@Terminal@.
...
- ~Асоціація файлів~@FileAssoc@
 ...
 також ~Special commands~@SpecCmd@
...
дочірнім процесам:
 ...
 #FARHOME# каталог, що містить ресурси far2l (наприклад, /usr/share/far2l)

 #FARLANG# назва поточної мови інтерфейсу

 #FARSETTINGS# ^<wrap>ім'я користувача, вказане у параметрі -u ~командного рядка~@CmdLine@

 #FARADMINMODE# ^<wrap>рівна "1", якщо FAR2L запущений від імені адміністратора (тобто, якщо його
ефективний ідентифікатор користувача дорівнює 0)

 #FARPID# Ідентифікатор процесу FAR2L

 Див. також розділ ~FAR2L: параметри командного рядка~@CmdLine@ про змінну оточення #FAR2L_ARGS#.
