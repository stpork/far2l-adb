...
`$^#©1996-2000 Eugene Roshal, ©2000-2016 FAR Group,' ©COPYRIGHTYEARS `FAR People'#
   ...
   ~Терминал~@Terminal@
   ~Внешний терминал: настройка~@ExternalTerminal@
   ...
   ~Способы запуска программ~@WaysToRunPrograms@
   ~Закладки~@Bookmarks@
...
@Help
$ # FAR2L: как пользоваться помощью#
...
@About
$ # FAR2L: что это такое#
    #FAR2L# - это работающая в текстовом режиме программа управления файлами и архивами, которая
...
(в настоящее время только на русском языке).

 #Режимы интерфейса#
    У FAR2L основные бекенды отрисовки (подробности в ~Режимы интерфейса~@UIBackends@):
        - #GUI|WX# (на базе wxWidgets) или #GUI|SDL# - работает в графическом режиме, #идеальный UX#
(возможны дополнительные зависимости к вашей desktop environment: wxWidgets toolkit и соответствующие пакеты);
        - #TTY|Xi# - работает в терминальном режиме, требует зависимости от нескольких библиотек X11
(для поддержки буфера обмена и всех-всех сочетаний клавиш в окружениях X11), #почти идеальный UX#;
        - #TTY|X# - работает в терминальном режиме, использует X11 для доступа к буферу обмена и для получения
состояний клавиш-модификаторов. Обеспечивает лучший UX, чем чистый TTY, но некоторые комбинации клавиш могут быть недоступны;
...
предоставляющих возможность доступа к буферу обмена и расширенную информацию о клавиатурных событиях, см. список ниже).
    ...
    Запуск far2l и выбор бекенда:
        ...
        - если у Вас far2l скомпилирован и с SDL и с WX, то принудительный запуск GUI|SDL: #far2l --SDL#
...
Если Ваш терминал не поддерживает последовательности OSC4, выключение данной опции позволяет
избежать появления в терминале артефактов после выхода из far2l.
...
@??ConfigDirs
 #Расположение персональных настроек FAR2L и истории#
    ...
    - некоторые файлы настроек (могут отсутствовать):
        ...
        - #favorites# - дополнительные пункты в ~меню перехода~@DriveDlg@ по Alt-F1/F2
...
использует её для улучшения возможностей интерфейса, если был собран с опцией TTYX/TTYXI.
...
far2l, который работает в режиме WX.
 ...
 #Параметры командной строки FAR2L:#
  #-a#
...
активной в данный момент панели. Доступны следующие режимы сортировки панели:
  ...
  #Замечания по числовой сортировке#
    ...
    FAR2L поддерживает два режима сортировки. Следующий пример показывает,
как сортируются файлы:

    Строковая сортировка             Числовая сортировка
...
размеры выбранных папок.
  ...
  Редактирование                                   #Ctrl-Shift-F4, F4#
...
сессии FAR2L, используется кодовая страница по умолчанию.
  ...
  Переименование или перенос файла под курсором             #Shift-F6#
    ...
    Для папки: если указанный путь (абсолютный или относительный) указывает на
существующую папку, исходная папка перемещается внутрь нее. В противном случае
папка переименовывается/перемещается по новому пути.
...
файловых ассоциаций.
  ...
  Если включена опция "~Использовать Ctrl-PgUp для меню перехода~@InterfSettings@",
нажатие #Ctrl-PgUp# в корневом каталоге переключает на сетевой плагин или
показывает ~меню перехода~@DriveDlg@.
...
$ #Команды управления панелями - прочие команды#
  Копирование текста с экрана                                #Alt-Ins#
    ...
    Для перемещения курсора используйте #клавиши управления курсором# или нажмите #левую кнопку мыши#.
    Для выбора текста используйте #клавиши управления курсором# при нажатой #Shift#
или нажатую #левую кнопку мыши#.
    Комбинация #Ctrl-A# или #A# выделяет весь экран.
    Комбинация #Ctrl-U# или #U# снимает выделение.
    #Enter#, #Ctrl-Ins#, #правая кнопка мыши# или двойное нажатие #левой кнопки мыши#
копируют выбранный текст в Буфер Обмена и завершает операцию.
    #Ctrl-<Gray +># добавляет выбранный текст к текущему содержанию Буфера Обмена
...
во внутренней ~Командной строке~@CmdLineCmd@, ~Ассоциациях файлов~@FileAssoc@,
~Меню пользователя~@UserMenu@ и действии ~Применить команду~@ApplyCmd@.

   #far:about#  - информация о far2l, список и информация о плагинах (также доступно через ~Меню команд~@CmdMenu@).

   #far:config# - ~редактор конфигурации~@FarConfig@ (также доступно через ~Меню команд~@CmdMenu@).
   ...
   #edit:[строка,колонка]file# или #edit:[строка]file# или #edit:[,колонка]file#
или #far:edit:[строка,колонка]file# - открыть в редакторе файл #file# и сразу перейти в заданную позицию.
Для этого впритык после двоеточия в квадратных скобках
...
выставить хотя бы один пробел: #edit: [1].txt#.
...
но некоторые доступны только отсюда или в конфигурационных ini-файлах.
 ...
 Параметры показаны в виде единого списка, состоящего из четырёх полей:
  ...
  #-# Сохраняется ли данная опция при сохранении конфигурации Far ('#c#' для общих, '#p#' для панелей) или нет ('#-#')
...
по папкам одного уровня.
    ...
    Стрелка #Влево# сворачивает текущую ветку. Если ветвь уже свернута, поднимается на один уровень выше.
    Стрелка #Вправо# раскрывает ветку дерева, свернутую при построении в соответствии
с заданной маской исключений или установленной глубиной сканирования.

    #Левый Ctrl+1#...#Левый Ctrl+0# разворачивает все ветви до выбранного уровня глубины (1...10).
...
$ #Меню файлов#
   ...
   #Архивные команды#   Выполнить команды управления архивом.
...
$ #Меню команд#
   ...
   #Сравнение файлов#     ~Сравнить содержимое текущих файлов~@FileDiff@
                        на активной и пассивной панелях.
                        Быстрый вызов #Ctrl+D# при пустой ком.строке.

   #Сравн. файлов#        ~Сравнить содержимое текущего файла~@FileDiff@
   #с одинак. именем#     на активной панели с файлом с тем же именем
                        на пассивной панели (если такой файл есть).
                        Быстрый вызов #Ctrl+Shift+D#.

   #Меню пользователя#    Позволяет редактировать главное или местное
                        ~меню пользователя~@UserMenu@. Для вставки пункта
                        используется #Ins# или #Ctrl+N#,
                        для удаления - #Del#, для редактирования - #F4#.
...
@ExternalTerminal
$ #Внешний терминал: настройка#
   Для запуска консольных приложений во внешнем эмуляторе терминала far2l использует служебный скрипт ~$FARHOME~@FAREnv@#/open.sh#.

   #Алгоритм выбора терминала по умолчанию.#

   1. Проверяется наличие #/etc/alternatives/x-terminal-emulator#. В дистрибутивах с механизмом альтернатив (Debian, Ubuntu, Red Hat) эта символическая ссылка указывает на приоритетный эмулятор терминала в системе. Если исполняемый файл существует, будет использован он.
   2. Во всех остальных случаях используется #xterm#.


   #Переопределение терминала.#

   Существует два основных способа изменить используемый эмулятор.
   1. Системный (для дистрибутивов с механизмом альтернатив).
      Вы можете изменить глобальный терминал по умолчанию, выполнив команду:
         #sudo update-alternatives --config x-terminal-emulator#
   2. Пользовательский (индивидуально для far2l).
      Cоздайте исполняемый файл #~~/.config/far2l/open.sh#. В файле определите переменную #$EXEC_TERM#, указав предпочтительный эмулятор терминала, например:
         #EXEC_TERM=kitty#
      Требование: выбранный терминал должен поддерживать флаг #-e#.
...
поддержку клавиатурных комбинаций (может быть #GUI|WX#, основанный на wxWidgets, или #GUI|SDL#).
...
соответствующий клавиатурный протокол).
    ...
    Список и ссылки на поддерживаемые терминалы смотрите в разделе ~Особенности FAR2L - начало работы~@Far2lGettingStarted??SupportedTerminals@.
...
подтверждения для следующих операций:
    ...
    - ~размонтирование~@DisconnectDrive@ из меню перехода;
...
пункт #Стандартные кодовые страницы#, то FAR2L будет использовать для поиска все
стандартные и #Любимые# кодовые страницы (список #Любимых# кодовых страниц можно
настроить в меню выбора кодовой страницы редактора или программы просмотра).
    Если перечень кодовых страниц, поиск по которым производится при выборе пункта
#Стандартные кодовые страницы#, является для вас избыточным, то вы можете,
при помощи клавиш #Ins# и #Space#, выбрать из списка стандартных и #Любимых#
кодовых страниц только те кодовые страницы, по которым вам необходимо
осуществлять поиск.
...
- только в текущей папке или в отмеченных папках
  (текущая версия FAR2L не ищет в каталогах,
  являющихся ~символическими ссылками~@HardSymLink@).
...
Допускаются следующие типы колонок:
    ...
    O[L]       - владелец файла
                 где: L - показывать имя домена (осталось с Windows);
    U          - группа файла
...
с заданной маской исключений или установленной глубиной сканирования.
    ...
    #Ctrl+Цифра# разворачивает все ветви до выбранного уровня глубины.
...
@TaskList
$ #Список задач#
    Список задач отображает активные задачи.

    #Del#                Завершение процесса: отправка #SIGTERM# и проверка, что процесс реально завершен
    #Shift-Del#          Завершение процесса: отправка #SIGKILL#
    #Ctrl-R#             Автообновление списка процессов вкл/выкл (если вкл, автообновление каждые 1 с)
    #t#/#T#                Сортировка по времени
    #n#/#N#                Сортировка по имени
    #i#/#I#                Сортировка по PID
    #c#/#C#                Сортировка по CPU
    #m#/#M#                Сортировка по памяти (Resident Set Size, RSS)
    #Ctrl-Alt-F#         Фильтровать элементы списка
    #Ctrl-F10#           Переход к каталогу #/proc/PID# на активной панели (только в Linux)
...
вставить новое вложенное меню или пункт меню с помощью #Ins# или #Ctrl+N# и редактировать
...
в связи с динамической природой подобных списков.
  ...
  Если эта опция включена, FAR2L будет автоматически сохранять конфигурацию при выходе из FAR.
Также будут сохраняться состояние и текущие папки обеих панелей.

  #Автосохранение состояния панелей#
  Состояние панелей (текущие папки, файлы, сортировка и т.п.) автоматически сохраняется при выходе из FAR.
...
$ #Настройка панели#
  ...
  #Автосмена каталога#      Если эта опция включена, то передвижения
  ...
  #Глубина сканирования#    Устанавливает максимальную глубину рекурсивного
                          сканирования каталогов при построении дерева.

  #Маска исключений#        Определяет ~маски~@FileMasks@ имен подкаталогов,
  #сканирования#            которые следует исключить при сканировании.
  #подкаталогов#            Используется для пропуска папок вроде .git или .mvn
                          при разворачивании дерева.
                          #Пример:# .git;.mvn;.svn;node_modules
...
ошибками.
  ...
  Отображать текстовый блок с версией far2l, копирайтами и подсказками по горячим клавишам под панелями при запуске.
...
специальные коды:
     ...
     $l - символ <
     $## - символ ## если пользователь - root, иначе $
     ...
     $@@xx - ^<wrap>"Администратор", если FAR2L запущен от root.
...
@FileDiff
$ #Сравнение файлов#
    FileDiff сравнивает файлы, выбранные на активной и пассивной файловых
панелях. Файл активной панели показан слева, файл пассивной панели — справа.
Изменённые строки и изменённые фрагменты внутри строк подсвечиваются. На
центральном разделителе для каждого блока изменений показаны действия слияния.
Стрелки копируют строки в другую панель, крестики удаляют лишние строки.

    Обе панели доступны для редактирования. Поддерживаются перенос строк,
номера строк, подсветка синтаксиса, поиск, работа с буфером обмена и обычные
команды редактора для отмены и повтора изменений.

    #F1#                 Показать эту справку
    #F2#                 Сохранить активный файл
    #F5#                 Объединить текущий блок изменений
    #Tab, Shift-Tab#     Перемещать фокус между левой панелью, разделителем и правой панелью
    #Left, Right#        Выбрать направление слияния, когда активен разделитель
    #Enter#              Выполнить выбранное слияние, когда активен разделитель
    #Ctrl-Up#            Перейти к предыдущему блоку изменений
    #Ctrl-Down#          Перейти к следующему блоку изменений
    #F7#                 Искать в активной панели
    #Shift-F7#           Продолжить поиск
    #Ctrl-Z#             Отменить изменение в активной панели
    #Ctrl-Shift-Z#       Повторить изменение в активной панели
    #F11#                Открыть меню плагинов редактора
    #Esc, F10#           Закрыть FileDiff

    Если активна текстовая панель, #F5# копирует текущий блок изменений из неё
в другой файл. Если активен разделитель, клавишами #Left# и #Right# выберите
действие, затем нажмите #Enter# или #F5#. Щелчок по стрелке копирует блок,
а щелчок по крестику удаляет лишние строки с этой стороны. Каждое слияние
записывается как одна операция отмены в целевой панели.

    Изменённые файлы помечаются символом #*#. При закрытии FileDiff предлагает
сохранить изменённые файлы. Если кодировка файла была определена эвристически,
перед первым сохранением FileDiff запрашивает подтверждение.
...
$ #Программа просмотра: клавиши управления#
   Команды программы просмотра
    ...
    2. В текущей версии FAR2L существует ограничение на максимальное
    ...
    3. FAR2L ~ищет~@ViewerSearch@ первое вхождение подстроки (#F7#) с начала видимой
       области экрана программы просмотра.

    4. Для автоматического скроллинга просматриваемого
       динамически обновляемого файла необходимо стать в
       конец файла (клавиша End).

    5. Нажатие Alt+PgUp/PgDn плавно увеличивает скорость скроллинга, последующее
...
@GrepFilter
    Здесь пользователь может временно отфильтровать содержимое просматриваемого файла, используя сопоставление с образцом утилиты #UNIX grep#.
Вы можете указать шаблон для сопоставления (или несколько шаблонов, разделенных #\|# - как в обычном grep) и/или шаблон,
который будет исключен из вывода.
Дополнительно можно просмотреть указанное #количество строк до или после совпавшего региона#, а также
использовать #регистрозависимое# сопоставление или сопоставлять #целые слова# вместо простой подстроки.
...
имеющегося на диске, то при этом будет создан новый файл.
	...
	1. ^<wrap>Если при создании нового файла указать имя несуществующей
папки, то будет показано предупреждение
"~Путь к редактируемому файлу не существует~@WarnEditorPath@".

	2. ^<wrap>При попытке повторного открытия в редакторе уже
редактируемого файла будет показано предупреждение
"~повторное открытие файла~@EditorReload@".

	3. ^<wrap>При создании нового файла по умолчанию используется кодировка
UTF-8, это поведение можно изменить в диалоге
~Настройки редактора~@EditorSettings@.

    #Элементы панели заголовка редактора#
   - Путь к файлу
   - #*# (файл изменен) или пусто
   - #-# (заблокирована модификация файла) или пусто      (переключается по #Ctrl-L#)
   - #"# (во время обработки #Ctrl-Q#) или пусто
   - #WW# (режим переноса строк) или пусто                (переключается по #F3# или #Alt-W# или в диалоге ~Настройки редактора~@EditorSettings@)
   - #Tn# (не преобразовывать Tab) или #Sn# (Tab в пробелы) (переключается по #Shift-F5# и #Ctrl-F5# или в диалоге ~Настройки редактора~@EditorSettings@)
   - #LF# или #CR# или #CRLF#: формат перевода строки         (переключается по #Shift-F2#)
   - Кодовая страница                                   (переключается по #F8# или #Shift-F8#)
   - Строка текущая/всего строк
   - Колонка текущая
   - #RSH# или пусто: атрибуты файла (R-только для чтения, S-системный, H-скрытый)
   - Код символа под курсором
   - Часы                                               (переключаются в диалоге ~Настройки интерфейса~@InterfSettings@)
  ...
  Операции над блоками
   ...
   Тянуть мышь,            Пометка блока
   удерживая левую кнопку
   #Ctrl-Shift-Клав.курсора# Пометка блока по словам
   #Alt-Клав.курсора#        Пометка вертикального блока (только при выключенном переносе строк)
   #Alt-Shift-Клав.курсора#  Пометка вертикального блока (стрелки с NumLock, только при выключенном переносе строк)
   #Alt# + тянуть мышь,      Пометка вертикального блока (только при выключенном переносе строк)
   удерживая левую кнопку
   ...
   #Ctrl-D#                  Удалить блок
   #Ctrl-P#                  ^<wrap>Копировать блок в текущую позицию курсора (только в режиме постоянных блоков)
   #Ctrl-M#                  ^<wrap>Перенести блок в текущую позицию курсора (только в режиме постоянных блоков)
  ...
  Прочие операции
   ...
   #F3# или #Alt-W#            Перенос строк (включён, выключен)
   #Ctrl-F3#                 Переключить отображение номеров строк
   ...
   #F9#                      Показать горизонтальное меню с списком доступных команд
   #Alt-F9#                  Изменение размеров окна FAR2L
   ...
   2. ^<wrap>Ввод текста или нажатие клавиш #BS#/#Del# применяется
к каждой строке выбранного вертикального блока.

   3. ^<wrap>Alt-Код символа на дополнительной клавиатуре вставляет в
редактируемую строку символ с указанным кодом (0-65535).

   4. ^<wrap>Если блок не выделен, то #Ctrl-Ins#/#Ctrl-C# помечают текущую
...
$ #Редактор: поиск и замена#
    ...
    В диалоге поиска есть также кнопка #Все#: по ней выполняется поиск сразу по
всему файлу, а все найденные вхождения показываются списком с номерами строк и
позиций. #Enter# в этом списке переводит курсор к выбранному вхождению, #Esc#
оставляет текущую позицию без изменений.
...
запрашивает привилегии суперпользователя.
    ...
    При включённой опции "~Использовать Ctrl-PgUp для меню перехода~@InterfSettings@",
...
~программы просмотра~@Viewer@.
  ...
  #Использовать для F3#     Запускать внешнюю программу просмотра по #F3#.

  #Использовать для Alt-F3# Запускать внешнюю программу просмотра по #Alt-F3#.
  ...
  #Распознавание URL#       Распознавать, подсвечивать и активировать
                          ссылки, которые начинаются с https:, http: или mailto:.
...
внешнего и ~встроенного редакторов~@Editor@.
  ...
  #Показывать номера#       Показывать номера строк слева от текста
  #строк#                   в редакторе. Также эта опция может
                          переключаться нажатием #Ctrl-F3# в редакторе.

  #Перенос по словам#       Переносить по словам. Также эта опция может
                          переключаться нажатием #F3# в редакторе.
...
(пропуская первые N байтов, равные размеру целевого файла);
...
исходный файл.
    ...
    Символические ссылки указывают на файлы и нелокальные папки, относительные пути
...
@WaysToRunPrograms
$ #Способы запуска программ без блокировки far2l#
  ...
  Запуск программ из far2l:
  - #program params &#: просто добавить в конце & - закроет программу после закрытия far2l;
  - #nohup program params &#: запуск производится программой nohup. В текущем каталоге создаётся файл nohup.out c выводом программы. Этот файл можно потом удалить. Такой запуск оставит программу запущенной после завершения far2l;
  - #nohup program params >/dev/null 2>&1 &#: оставит программу запущенной после завершения far2l без лишнего вывода;
  - #setsid program params >/dev/null 2>/dev/null#: при использовании в примерах выше & некоторые программы при закрытии выводят в текущий терминал информацию о своём завершении - использование setsid позволяет избежать такого засорения вывода текущей программы;
