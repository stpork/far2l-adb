	...
	void DraggingMove(COORD pos)
	{
		if (!_dragging) {
			...
			if (!WINPORT(GetConsoleImageCaps)(NULL, sizeof(_drag_wgi), &_drag_wgi)) {
				DBG("GetConsoleImageCaps failed");
...
static LONG_PTR WINAPI ImageDlgProc(HANDLE hDlg, int Msg, int Param1, LONG_PTR Param2)
{
	switch (Msg) {
		...
		case DN_KEY:
		{
			...
			const bool has_alt = (((int)Param2) & KEY_ALT) != 0;
			...
			switch (key) {
				...
				case KEY_MSWHEEL_UP: iv->Scale(delta); break;
				case KEY_MSWHEEL_DOWN: iv->Scale(-delta); break;
				// Pan with Alt+arrows
				case KEY_NUMPAD6: if (has_alt) iv->Shift(delta, 0); break;
				case KEY_NUMPAD4: if (has_alt) iv->Shift(-delta, 0); break;
				case KEY_NUMPAD2: if (has_alt) iv->Shift(0, delta); break;
				case KEY_NUMPAD8: if (has_alt) iv->Shift(0, -delta); break;
				case KEY_NUMPAD9: if (has_alt) iv->Shift(delta, -delta); break;
				case KEY_NUMPAD1: if (has_alt) iv->Shift(-delta, delta); break;
				case KEY_NUMPAD3: if (has_alt) iv->Shift(delta, delta); break;
				case KEY_NUMPAD7: if (has_alt) iv->Shift(-delta, -delta); break;
				// Navigation: arrows, Up/Down = first/last
				case KEY_RIGHT: if (has_alt) iv->Shift(delta, 0); else iv->Iterate(true); break;
				case KEY_LEFT: if (has_alt) iv->Shift(-delta, 0); else iv->Iterate(false); break;
				case KEY_DOWN: if (has_alt) iv->Shift(0, delta); else iv->Last(); break;
				case KEY_UP: if (has_alt) iv->Shift(0, -delta); else iv->Home(); break;
				// Zoom with PgUp/PgDn (Shift for precise 1%)
				case KEY_PGUP: iv->Scale(shift ? 1 : delta); break;
				case KEY_PGDN: iv->Scale(shift ? -1 : -delta); break;
				// Rotation: Home/End = 90°, Shift+Home/Shift+End = 1°
				case KEY_HOME: if (shift) iv->RotateFine(-1); else iv->Rotate(-90); break;
				case KEY_END: if (shift) iv->RotateFine(1); else iv->Rotate(90); break;
				case KEY_TAB: iv->Rotate(shift ? -90 : 90); break;
		...
		case DN_ENTERIDLE:
		{
			...
			if (iv && iv->_first_draw) {
				...
				// Apply async full-resolution decode if one completed in background
				iv->ApplyPendingFullRes();
			}
		}
		break;
...
static EXITED_DUE ShowImageAtFullInternal(size_t initial_file, std::vector<std::pair<std::string, bool> > &all_files, std::unordered_set<std::string> *selection, bool silent_exit_on_error, size_t *final_file = nullptr)
{
	...
	for (;;) {
		SMALL_RECT screen_rc;
		g_far.AdvControl(g_far.ModuleNumber, ACTL_GETFARRECT, &screen_rc, 0);

		SMALL_RECT rc = screen_rc;  // Default to full screen

		// For compact frame mode, preload image and calculate preferred size
		if (g_settings.CompactFrame()) {
			if (iv.Preload()) {
				rc = iv.GetPreferredRect(screen_rc);
			}
		}
		...
		FarDialogItem DlgItems[] = {
			{ DI_DOUBLEBOX, 0, 0, rc.Right - rc.Left, rc.Bottom - rc.Top, FALSE, {}, DIF_SHOWAMPERSAND, 0, L"???", 0 },
			{ DI_SINGLEBOX, 0, 0, rc.Right - rc.Left, rc.Bottom - rc.Top, FALSE, {}, DIF_HIDDEN | DIF_SHOWAMPERSAND, 0, L"???", 0 },
			{ DI_USERCONTROL, 1, 1, rc.Right - rc.Left - 1, rc.Bottom - rc.Top - 1, 0, {COL_DIALOGBOX}, 0, 0, L"", 0},
			{ DI_TEXT, 0, rc.Bottom - rc.Top, rc.Right - rc.Left, rc.Bottom - rc.Top, 0, {}, DIF_CENTERTEXT | DIF_SHOWAMPERSAND, 0, hint.c_str(), 0},
			{ DI_TEXT, 1, 0, 1, 0, 0, {}, DIF_SHOWAMPERSAND, 0, L"", 0},
			{ DI_TEXT, rc.Right - rc.Left - 1, 0, rc.Right - rc.Left - 1, 0, 0, {}, DIF_SHOWAMPERSAND, 0, L"", 0},
		};

		HANDLE dlg = g_far.DialogInit(g_far.ModuleNumber, rc.Left, rc.Top, rc.Right - rc.Left, rc.Bottom - rc.Top,
...
EXITED_DUE ShowImageAtFull(size_t initial_file, std::vector<std::pair<std::string, bool> > &all_files, std::unordered_set<std::string> &selection, bool silent_exit_on_error, size_t *final_file)
{
	return ShowImageAtFullInternal(initial_file, all_files, &selection, silent_exit_on_error, final_file);
}
